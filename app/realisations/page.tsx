    ...
    <>
      ...
      <section className="py-16 relative overflow-hidden" style={{ background: 'var(--bg-1)' }}>
        <div className="max-w-[1200px] mx-auto px-6">
          ...
          {/* Filter strip */}
          <div className="flex flex-wrap items-center gap-2 mb-14" style={{ borderBottom: '1px solid rgba(28,20,24,0.1)', paddingBottom: '20px' }}>
            <span className="text-[10px] uppercase tracking-[0.3em] mr-4" style={{ color: 'rgba(28,20,24,0.22)' }}>Filtrer</span>
            {categories.map((cat) => (
              <button
                key={cat.id}
                ...
                style={{
                  ...
                  color: activeCategory === cat.id ? '#fff' : 'rgba(28,20,24,0.4)',
                  border: activeCategory === cat.id ? '1px solid transparent' : '1px solid rgba(28,20,24,0.1)',
                }}
              >
                {cat.label}
              </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {filtered.map((project, i) => (
                ...
                >
                  {/* Number */}
                  <span className="font-mono text-[11px] shrink-0 tabular-nums pt-1" style={{ color: 'rgba(28,20,24,0.18)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                      ...
                      <span className="text-[10px]" style={{ color: 'rgba(28,20,24,0.25)' }}>{project.year}</span>
                      {project.featured && (
                    ...
                    <p className="text-sm mb-3 leading-relaxed" style={{ color: 'rgba(28,20,24,0.42)' }}>
                      {project.description}
                    </p>
                    <div className="flex flex-wrap gap-4 text-[11px]" style={{ color: 'rgba(28,20,24,0.32)' }}>
                    ...
                    {project.results.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-y-1">
                        {project.results.slice(0, 2).map((r) => (
                          <span key={r} className="mr-6 text-[11px] flex items-start gap-1.5" style={{ color: 'rgba(28,20,24,0.38)' }}>
              ...
              {filtered.length === 0 && (
                <div className="py-20 text-center">
                  <p className="text-sm" style={{ color: 'rgba(28,20,24,0.3)' }}>Aucun projet dans cette catégorie.</p>
                </div>
              )}
            </motion.div>
          </AnimatePresence>
      ...
      {/* ── STATS ROW ── */}
      <section className="py-16 relative overflow-hidden" style={{ background: 'var(--bg-0)', borderTop: '1px solid rgba(28,20,24,0.08)' }}>
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              >
                ...
                <p className="text-[10px] uppercase tracking-[0.25em]" style={{ color: 'rgba(28,20,24,0.32)' }}>{s.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
