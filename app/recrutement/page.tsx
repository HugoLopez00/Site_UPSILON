    ...
    <>
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce} className="lg:w-[64%]">
              ...
              {benefits.map((b, i) => (
                <motion.div key={b.title} variants={fadeInUp} transition={{ delay: i * 0.07 }}
                  className="group flex items-start gap-5 py-6" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                  <span className="font-mono text-[11px] shrink-0 tabular-nums pt-0.5" style={{ color: 'rgba(240,240,244,0.18)' }}>
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-relaxed" style={{ color: 'rgba(240,240,244,0.42)' }}>{b.description}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
      ...
      <section className="py-24 relative overflow-hidden" style={{ background: 'var(--bg-0)' }}>
        ...
        <div className="absolute inset-0 flex items-center overflow-hidden pointer-events-none select-none" aria-hidden>
          <span className="font-black uppercase leading-none" style={{ fontSize: 'clamp(5rem, 16vw, 18rem)', letterSpacing: '-0.06em', color: 'rgba(255,255,255,0.018)', whiteSpace: 'nowrap' }}>
            Postes
          </span>
        </div>
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce} className="lg:w-[64%]">
              ...
              {openRoles.map((role, i) => (
                <motion.div key={role.role} variants={fadeInUp} transition={{ delay: i * 0.07 }}>
                  ...
                  >
                    <span className="font-mono text-[11px] shrink-0 tabular-nums" style={{ color: 'rgba(240,240,244,0.18)' }}>{String(i+1).padStart(2,'0')}</span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-[10px] font-medium px-2.5 py-1 rounded-lg"
                        style={{ background: 'var(--bg-2)', border: '1px solid rgba(255,255,255,0.07)', color: 'rgba(240,240,244,0.4)' }}>
                        {role.pole}
                      </span>
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce} className="lg:w-[64%]">
              ...
              {processSteps.map((step, i) => (
                <motion.div key={step.number} variants={fadeInUp} transition={{ delay: i * 0.08 }}
                  className="flex items-start gap-5 py-6" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm leading-relaxed" style={{ color: 'rgba(240,240,244,0.42)' }}>{step.description}</p>
                  </div>
                  <span className="text-[10px] font-bold uppercase tracking-wide shrink-0" style={{ color: 'rgba(240,240,244,0.25)' }}>{step.duration}</span>
                </motion.div>
              ))}
            </motion.div>
          </div>
            ...
            <div className="lg:w-[64%]">
              {submitted ? (
                ...
                >
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: 'rgba(240,240,244,0.4)' }}>École / Formation *</label>
                    ...
                    <div>
                      <label className="block text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: 'rgba(240,240,244,0.4)' }}>Année d&apos;études *</label>
                      <select required value={formData.year}
                        ...
                        style={{ background: 'var(--bg-2)', border: '1px solid rgba(255,255,255,0.08)', color: formData.year ? 'var(--text)' : 'rgba(240,240,244,0.3)' }}>
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: 'rgba(240,240,244,0.4)' }}>Pôle souhaité *</label>
                    <select required value={formData.pole}
                      onChange={(e) => setFormData({ ...formData, pole: e.target.value })}
                      className="w-full rounded-xl px-4 py-3 text-sm focus:outline-none transition-colors appearance-none"
                      style={{ background: 'var(--bg-2)', border: '1px solid rgba(255,255,255,0.08)', color: formData.pole ? 'var(--text)' : 'rgba(240,240,244,0.3)' }}>
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: 'rgba(240,240,244,0.4)' }}>Lettre de motivation *</label>
